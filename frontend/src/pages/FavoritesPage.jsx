...
export default function FavoritesPage() {
  ...
  const [share, setShare] = useState(null)         // { cid, url, expiryDays, loading }
  ...
  const doShare = async (cid, expiryDays = 30) => {
    setShare({ cid, url: null, expiryDays, loading: true })
    try {
      const res = await api.shareFavCollection(cid, expiryDays)
      setShare({ cid, url: res.shareUrl || `${window.location.origin}/share/${res.token}`, expiryDays, loading: false })
    } catch (e) {
      setShare({ cid, url: null, expiryDays, loading: false, error: e.message })
    }
  }
            ...
            <>
              <div className="fav-col-bar">
                ...
                <div className="fav-col-bar-actions">
                  <button className="btn btn-primary btn-sm" onClick={() => doShare(active.id, 30)} disabled={!(active.items || []).length}>
                    Compartir
                  </button>
              ...
              {share && share.cid === active.id && (
                <div className="fav-share-box">
                  {share.loading && <span className="fav-share-loading">Generando enlace…</span>}
                  {share.error && <span className="field-error">{share.error}</span>}
                  {share.url && (
                    <>
                      <div className="fav-share-row">
                        <span className="fav-share-hint">Enlace público (sin login) · caduca en {share.expiryDays} días</span>
                      ...
                      <div className="fav-share-exp">
                        {[7, 30, 90].map(d => (
                          <button key={d} className={`filter-chip ${share.expiryDays === d ? 'active' : ''}`} onClick={() => doShare(active.id, d)}>{d} días</button>
                        ))}
                      </div>
                    </>
                  )}
